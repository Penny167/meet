...
Meet app was developed using a test-driven approach. Further details about the approach, test runner,packages and files are included in the Testing section below.

Please note that as this project was built for educational purposes, the events themselves are fictitious. The app is published to Github pages and can be viewed using the link below.
...
- Finally, **inside the auth-server directory**, create a config.json file. Inside the file, create the following object, replacing the values with the credentials you obtained from Google and your project ID (you will find the project ID on your Google dashboard):<br>
{
...
At this stage the project already contains an auth-server directory and the serverless functions used in the authorisation process, however, these need to be deployed to your own AWS Lambda service.
...
```
- Now in the auth-server directory, delete the .serverless folder as you will replace this when the functions are (re)deployed to your own AWS account.
...
```
- Finally, to deploy the serverless functions to AWS Lambda, inside the auth-server directory run:
```
serverless deploy
```
<br>

**4) Replace existing code and credentials**
At this stage all of your local files should be updated to run the app on your own remote server and hosted on your own live website linked to your gitub repository. To deploy the app:
- Commit all of your changes and push to your main github branch then run:
```
npm run deploy
```
Your app will now be live at your github pages URL!
